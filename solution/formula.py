...
class Token(list):
    ...
    def is_function(self) -> bool:
        return self.tok_type == 'func' 

    def is_para(self) -> bool:
        return self.tok_type == 'para'
...
class FormulaTree(Tree):
    ...
    def iter_subtree_with_address(self) -> Iterator[tuple[list[int], FormulaTree]]:
        ...
        for idx, child in enumerate(self.children):
            for addr, sub in child.iter_subtree_with_address():
                yield [idx] + addr, sub 
    ...
    def replace_subtree(self, src: FormulaTree, dest: FormulaTree):
        ...
        """
        for addr, sub in self.iter_subtree_with_address():
            if sub == src:
                self.delete(addr)
                self.insert(addr, dest)
    ...
    def __eq__(self, other: FormulaTree) -> bool:
        ...
        """
        if isinstance(other, FormulaTree):
            if self.datum == other.datum and len(self.children) == len(other.children):
                return all([s_child == o_child for s_child, o_child in zip(self.children, other.children)])
            else:
                return False    
        return False      
    ...
    def flatten(self) -> str:
        ...
        Step-by-Step Implementation Guide:
            ...
            3. Check if the current node's datum represents a unary operator:
                ...
                - Return the resulting string.
        """
        if self.datum.is_varaible() or self.datum.variable.is_number():
            return self.datum.tok_val 
        elif self.datum.is_operator():
            for child in self.children:
                res.append(child.flatten())
            return self.datum.tok_val.join(res)
        elif self.datum.is_unary_operator():
            assert len(self.children) == 1
            return self.datum.tok_val + self.children[0].flatten()
...
class Formula:
    ...
    def substitute(self, var: str, val: number) -> Formula:
        ...
        tree = self.tree
        new_tree = tree.replace_subtree(FormulaTree(root = Token('var', var)),
                                        FormulaTree(root = Token('num', val)))
        return Formula(new_tree.flatten())
    ...
    def __call__(self, *value_list, **value_dict) -> number:
        ...
        if value_dict == {}:
            ...
            assert self.variables == set(value_dict.keys()), \
                    f'Not a valid input, {self.variables} / {set(value_dict.keys())}'
    ...
    def _calculate(tree, value_dict: Dict[str, number]) -> number:
        ...
        """
        tok_type, tok_val = tree.tok_type, tree.tok_val 
        if tok_type == 'op':
            args = [Formula._calculate(c, value_dict) for c in tree.children]
            return Token.OPERATION_WITH_FUNCTIONS[tok_val](*args)
        elif tok_type == 'num':
            return tok_val
        elif tok_type == 'var':
            return value_dict[tok_val]
        else:
            assert False, 'TypeError : invalid type %s'%str(tok_val)
        ...
        for tok in tok_strings:
            if tok in tokens['num']:
                if re.match(tok, left) is not None:
                    m = re.match(tok, left)
                    res.append(Token('num', float(left[m.start():m.end()])))
                    left = left[m.end():]
                    num_flag = True
            elif tok in tokens['comma']:
                if re.match(tok, left) is not None:
                    m = re.match(tok, left)
                    res.append(Token('comma', left[m.start():m.end()]))
                    left = left[m.end():]
            elif tok in tokens['var']:
                if re.match(tok, left) is not None:
                    m = re.match(tok, left)
                    res.append(Token('var', left[m.start():m.end()]))
                    left = left[m.end():]
                    num_flag = True
            else:
                if left.startswith(tok):
                    ...
                    if len(k) == 1:
                        yield Token(k[0], left[0])
                    else:
                        if num_flag:
                            res.append(Token('op', '+'))
                            res.append(Token('unary', 'unary %s'%left[0]))
                        else:
                            res.append(Token('unary', 'unary %s'%left[0]))
                    left = left[1:]        
                    num_flag = False
...
def compress_tok(tokenizer: list[Token]) -> list[Token]:
    ...
    for token in tokenizer:
        ...
        if tok_type == 'var' and tok_val in Token.FUNCTION_DICT.keys():
            res.append(Token('func', tok_val))
        else:
            res.append(Token(tok_type, tok_val))
    return res 
...
def expr(operator: Stack, operand: Stack, tokens: list[Token], idx: int) -> int:
    ...
    if idx != len(tokens):
        ...
        while next_tok.tok_val in Token.TOKEN_TABLE['op']:
...
def part(operator: Stack, operand: Stack, tokens: list[Token], idx: int) -> int:
    ...
    if next_tok.is_number() or next_tok.is_variable():
        # part := num | var
        operand.push(FormulaTree(root = next_tok))
    elif next_tok.is_para():
        # part := "(" expr ")"
        ...
        operand.push(e)
        tokens = tokens[idx:]
    elif next_tok.is_unary_operator():
        # part := unary part
        push_operator(operator, operand, next_tok)
        idx += 1
        idx = part(operator, operand, tokens, idx)
    elif next_tok.is_function():
        # part := func "(" (expr ,)* expr ")"
        idx += 1
        tokens_in_para = tokens[idx+1:find_match(tokens, idx)]
        args = [[]]
        ...
        for tok_type, tok in tokens_in_para:
            if tok_type == 'comma':
                ...
                args[-1].append(Token(tok_type, tok))

        args = [recursive_descent(arg) for arg in args]
        idx = find_match(tokens, idx)

        operand.push(FormulaTree(root = next_tok, children = args))
    else:
...
def pop_operator(operator: Stack, operand: Stack) -> None:
    ...
    if top.is_operator():
        arg1 = operand.pop()
        arg2 = operand.pop()
        operand.push(FormulaTree(root = top, children = [arg2, arg1]))
    elif top.is_unary_operator():
        arg1 = operand.pop()

        if isinstance(arg1, FormulaTree):
            if (arg1.children == [] and arg1.tok_type == 'num'):
                operand.push(FormulaTree(root = Token('num', -1*arg1.tok_val)))
            else:
                operand.push(FormulaTree(root = Token('op', '*'), \
                        children = [FormulaTree(root = Token('num', -1)), arg1]))
        else:   
            operand.push(FormulaTree(root = Token('num', -1*arg1.tok_val)))
    else:
...
def push_operator(operator: Stack, operand: Stack, op: Token) -> None:
    ...
    if not operator.is_empty():
        top = operator.top()
        while Token.OPERATOR_PRIORITY[top.tok_val] > Token.OPERATOR_PRIORITY[op.tok_val]:
            pop_operator(operator, operand)
            top = operator.top()
            if top is None:
                break
